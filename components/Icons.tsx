/**
 * @file Icons.tsx
 * @description 이 파일은 애플리케이션 전체에서 사용되는 모든 SVG 아이콘들을 React 컴포넌트 형태로 모아놓은 곳입니다.
 * SVG 코드를 직접 JSX 안에 넣는 대신, 이렇게 별도의 컴포넌트로 만들어두면 다음과 같은 장점이 있습니다.
 * 1. **코드 가독성 향상**: 복잡한 SVG 코드가 JSX 파일에서 사라져 깔끔해집니다.
 * 2. **재사용성**: `<HomeIcon />` 처럼 간단하게 아이콘을 여러 곳에서 재사용할 수 있습니다.
 * 3. **쉬운 커스터마이징**: props를 통해 아이콘의 크기, 색상 등을 쉽게 변경할 수 있습니다.
 * 4. **관리 용이성**: 모든 아이콘이 한 파일에 모여 있어 관리가 편리합니다.
 */

// React 라이브러리를 가져옵니다.
import React from 'react';

// 모든 아이콘 컴포넌트가 공통으로 받을 수 있는 props의 타입을 정의합니다.
// `React.SVGProps<SVGSVGElement>`는 `<svg>` 태그가 가질 수 있는 모든 표준 속성(예: className, onClick, width, height 등)을 포함합니다.
type IconProps = React.SVGProps<SVGSVGElement>;

// 아래는 각 아이콘을 나타내는 React 함수형 컴포넌트들입니다.
// 각 컴포넌트는 IconProps 타입의 props를 받아서,
// `{...props}` 구문을 통해 부모로부터 받은 모든 속성을 그대로 `<svg>` 태그에 적용합니다.
// 이렇게 하면 아이콘을 사용할 때 <HomeIcon className="h-6 w-6 text-blue-500" /> 처럼 크기나 스타일을 자유롭게 지정할 수 있습니다.
...
export const LanguageIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="m10.5 21 5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 0 1 6-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 0 1-3.827-5.802" /></svg>
);

export const CalculatorIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75V18m-7.5-6.75h.008v.008H8.25v-.008Zm0 2.25h.008v.008H8.25V11.25Zm0 2.25h.008v.008H8.25v-.008Zm0 2.25h.008v.008H8.25V16.5Zm2.25-4.5h.008v.008H10.5v-.008Zm0 2.25h.008v.008H10.5V11.25Zm0 2.25h.008v.008H10.5v-.008Zm0 2.25h.008v.008H10.5V16.5Zm2.25-4.5h.008v.008H12.75v-.008Zm0 2.25h.008v.008H12.75V11.25Zm0 2.25h.008v.008H12.75v-.008Zm0 2.25h.008v.008H12.75V16.5Z" /><path strokeLinecap="round" strokeLinejoin="round" d="M9 3.75H4.5A2.25 2.25 0 0 0 2.25 6v13.5A2.25 2.25 0 0 0 4.5 21h15a2.25 2.25 0 0 0 2.25-2.25V6A2.25 2.25 0 0 0 19.5 3.75h-4.5m0-3v3m0 0h-3m3 0a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75V.75a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v3Zm-3 0V.75m0 3v3m0-3h3m-3 0h-3" /></svg>
);
...
export const BanknotesIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0 1 15.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 0 1 3 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 0 0 .75-.75v-.75m0 0H3.75m0 0h-.375a1.125 1.125 0 0 1-1.125-1.125V6.75m0 0h18" /><path strokeLinecap="round" strokeLinejoin="round" d="M6 12h12M6 15h12M6 18h12" /></svg>
);

export const UserGroupIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m-7.5-2.962c.57-1.023-.19-2.057-1.28-2.057a3 3 0 0 0-4.682 2.72 9.094 9.094 0 0 0 3.741.479m-4.535 2.962c.57.983 1.606 1.657 2.76 1.657s2.19-.674 2.76-1.657m-4.535 2.962 1.258-2.156a27.11 27.11 0 0 1 2.018-2.156 1.125 1.125 0 0 1 1.096-.088l.842.42a1.125 1.125 0 0 0 1.096-.088l2.018-2.156a27.11 27.11 0 0 0 1.258-2.156m-4.535 2.962a27.11 27.11 0 0 0 2.018-2.156" /><path strokeLinecap="round" strokeLinejoin="round" d="M12 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm12 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" /></svg>
);
...
export const WrenchScrewdriverIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.83-5.83M11.42 15.17l2.472-2.472a.563.563 0 0 1 .796 0l2.472 2.472a.563.563 0 0 1 0 .796l-2.472 2.472a.563.563 0 0 1-.796 0l-2.472-2.472a.563.563 0 0 1 0-.796Zm-2.828 2.828a.563.563 0 0 1 0-.796l2.472-2.472a.563.563 0 0 1 .796 0l2.472 2.472a.563.563 0 0 1 0 .796l-2.472 2.472a.563.563 0 0 1-.796 0l-2.472-2.472ZM3 11.25a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm3-9a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm-3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm-3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Zm-3.75 0a.75.75 0 0 1 .75-.75h1.5a.75.75 0 0 1 .75.75v1.5a.75.75 0 0 1-.75.75h-1.5a.75.75 0 0 1-.75-.75v-1.5Z" />
  </svg>
);
...
export const SelectorIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 15 12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" /></svg>
);
...
export const BriefcaseIcon: React.FC<IconProps> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M20.25 14.15v4.075a2.25 2.25 0 0 1-2.25 2.25h-13.5a2.25 2.25 0 0 1-2.25-2.25V14.15M16.5 18.75h-9M9.75 14.15v-4.875a1.125 1.125 0 0 1 1.125-1.125h2.25a1.125 1.125 0 0 1 1.125 1.125V14.15" />
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 13.5h.008v.008H12v-.008Z" />
        <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5v6.375a2.25 2.25 0 0 1-2.25 2.25h-13.5a2.25 2.25 0 0 1-2.25-2.25V6.75Z" />
    </svg>
);

export const Cog6ToothIcon: React.FC<IconProps> = (props) => (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-1.003 1.11-1.226.554-.223 1.197-.245 1.752-.07C13.044 2.828 13.5 3.336 13.5 3.943v.272c.42.081.826.21 1.22.388.527.234 1.018.593 1.442.98.41.375.753.818.97 1.306.225.498.333 1.025.333 1.562v.272c-.081.42-.21.826-.388 1.22-.234.527-.593 1.018-.98 1.442-.375.41-.818.753-1.306.97-.498.225-1.025.333-1.562.333h-.272c-.09.542-.56 1.003-1.11 1.226-.554-.223-1.197.245-1.752.07C10.956 21.172 10.5 20.664 10.5 20.057v-.272c-.42-.081-.826-.21-1.22-.388-.527-.234-1.018-.593-1.442-.98-.41-.375-.753-.818-.97-1.306-.225-.498-.333-1.025-.333-1.562v-.272c.081-.42.21-.826.388-1.22.234-.527.593 1.018.98-1.442.375-.41.818-.753 1.306-.97.498-.225 1.025-.333 1.562-.333h.272ZM12 15.75a3.75 3.75 0 1 0 0-7.5 3.75 3.75 0 0 0 0 7.5Z" />
    </svg>
);
...
export const PrinterIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M6.72 13.829c-.24.03-.479.06-.721.06M6.72 13.829a2.25 2.25 0 0 0-1.51-2.26m0-4.575a2.25 2.25 0 0 1 1.51-2.26m0 0c.584-.049 1.17-.053 1.76-.053m0 0h5.038c.59 0 1.176.004 1.76.053m0 0a2.25 2.25 0 0 1 1.51 2.26m0 4.575a2.25 2.25 0 0 0-1.51 2.26m0 0c-.584.049-1.17.053-1.76.053m0 0h-5.038c-.59 0-1.176-.004-1.76-.053m12.338-12.838a2.25 2.25 0 0 0-1.511-2.26m0 0c-.584-.049-1.17-.053-1.76-.053H7.72c-.59 0-1.176.004-1.76.053a2.25 2.25 0 0 0-1.51 2.26M6 19.5h12a2.25 2.25 0 0 0 2.25-2.25V9.75H3.75V17.25A2.25 2.25 0 0 0 6 19.5Z" />
  </svg>
);

export const DocumentDownloadIcon: React.FC<IconProps> = (props) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3-8.25H6a2.25 2.25 0 0 0-2.25 2.25v12A2.25 2.25 0 0 0 6 20.25h12A2.25 2.25 0 0 0 20.25 18V9.75a2.25 2.25 0 0 0-2.25-2.25m-9 6h.008v.008H9v-.008Zm0 3h.008v.008H9V15Zm0 3h.008v.008H9v-.008Z" />
  </svg>
);
